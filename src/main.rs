...
fn test_load_step() {
    let cases : &[(Str, Option<usize>)] = &[
        ...
        (" Then images <output> and <groundtruth> are visually identical", Some(2)),
    ...
    for (input, expectation) in cases {
        match (Step::from_str(input),expectation) {
            ...
            (None, Some(arity)) => {
                panic!("Expected to parse the following as a step with arity {}:\n{}", arity, input);
            }
            (Some(step), None) => {
                dbg!(input);
                dbg!(step);
...
fn test_load_scenario_outline() {
    let input = 
r###"Scenario Outline: Shave an animal
